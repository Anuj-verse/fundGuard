...
from app.security import SecureInferenceLayer

security = SecureInferenceLayer()
...
def _fallback_response(
    ...
    similar_summary = (
        "; ".join(case["summary"] for case in similar_cases)
        if similar_cases
        else "No comparable cases were retrieved."
    )

    return {
        "investigation_summary": (
            f"Account {request.account_id} shows elevated fraud indicators "
            f"with risk score {request.risk_score:.2f}."
        ),

        "risk_rationale": (
            f"Pattern flags: "
            f"{', '.join(request.pattern_flags) if request.pattern_flags else 'none'}. "
            f"Similar cases: {similar_summary}"
        ),

        "str_draft": (
            "<STR><CaseId>{case_id}</CaseId>"
            "<AccountId>{account_id}</AccountId>"
            "<RiskScore>{risk_score}</RiskScore>"
            "<Narrative>Manual review recommended.</Narrative></STR>"
        ).format(
            case_id=request.case_id or request.account_id,
            account_id=request.account_id,
            risk_score=request.risk_score,
        ),
    }
...
def generate_explanation(
    ...
    try:
        ...
        if LLM_PROVIDER == "groq":
            ...
            response = security.secure_inference(
                prompt,
                ...
                lambda p: client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You write structured fraud investigation outputs."
                        },
    ...
    return ExplainResponse(
        ...
        risk_rationale=str(
            payload.get("risk_rationale", "")
        ),

        str_draft=str(
            payload.get("str_draft", "")
        ),
